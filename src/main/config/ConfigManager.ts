 ...
 */
type ConfigValue = string | number | boolean | null | string[] | { [key: string]: ConfigValue };
...
export class ConfigManager {
    ...
    static async initialize(): Promise<void> {
        ...
        try {
            ...
            console.error(
                'Failed to access encryption key. Device environment may have changed.',
                error,
            );
    ...
    static async set(updates: ConfigRecord): Promise<string[]> {
        ...
        for (const [key, value] of Object.entries(updates)) {
            ...
            const encrypted = await encryption.encryptField(this.encryptionKey!, jsonValue);
