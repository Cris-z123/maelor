 ...
 */
export async function encrypt(key: CryptoKey, plaintext: string | Buffer): Promise<EncryptedData> {
    const iv = generateIV();
 ...
 */
export async function decrypt(key: CryptoKey, encryptedData: EncryptedData): Promise<string> {
 ...
 */
export async function hmacSha256(key: CryptoKey, data: string): Promise<string> {
    const buffer = new TextEncoder().encode(data);
    const hmac = await webcrypto.subtle.sign('HMAC', key, buffer);
