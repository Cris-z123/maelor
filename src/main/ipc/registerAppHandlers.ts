...
export async function registerAppIpcHandlers(
  ...
  ipcMain.handle(IPC_CHANNELS.ONBOARDING_DETECT_EMAIL_CLIENT, async () => deps.detectOutlookDirectory());
  ipcMain.handle(IPC_CHANNELS.ONBOARDING_VALIDATE_EMAIL_PATH, async (_event, directoryPath: string) =>
    deps.validateOutlookDirectory(directoryPath)
  );
  ipcMain.handle(
    IPC_CHANNELS.ONBOARDING_TEST_LLM_CONNECTION,
  ...
  ipcMain.handle(
    IPC_CHANNELS.RUNS_GET_BY_ID,
    async (_event, request: { runId: string }) => deps.getRunById(request.runId)
  );
  ipcMain.handle(
    IPC_CHANNELS.RUNS_LIST_RECENT,
    async (_event, request?: { limit?: number }) => deps.listRecentRuns(request?.limit)
  );
