...
const COMPLETED_PATTERN =
    /\b(done|completed|resolved|closed|finished)\b|已完成|完成了|已解决|已关闭/i;
...
function createFingerprint(
    ...
    return createHash('sha256')
        .update(
            `${messageIdentifier ?? 'no-message-id'}|${sentAt ?? 0}|${senderAddress ?? 'unknown'}|${subject}`,
        )
        .digest('hex');
}
...
function createActionItemFromEmail(email: ParsedPstEmail): PersistedActionItemRecord | null {
    ...
    const itemType: PersistedActionItemRecord['itemType'] = COMPLETED_PATTERN.test(haystack)
        ? 'completed'
        : 'todo';
...
function mapPstMessage(pstFilePath: string, message: PSTMessage): ParsedPstEmail {
    ...
    const senderAddress =
        normalizeWhitespace(
            message.senderEmailAddress ||
                message.sentRepresentingEmailAddress ||
    ...
    const messageIdentifier =
        normalizeWhitespace(message.internetMessageId || '').replace(/^<|>$/g, '') || null;
...
export class RunExecutionService {
    ...
    private async execute(): Promise<RunExecutionResult> {
        const loadSettings =
            this.dependencies.loadSettings ?? (() => RunRepository.getSettingsSeed());
        const validateDirectory =
            this.dependencies.validateDirectory ??
            ((directoryPath: string) => PstDiscovery.validateDirectory(directoryPath));
        const loadPstEmails =
            this.dependencies.loadPstEmails ??
            ((pstFile: ValidationFile) => this.loadEmailsFromPst(pstFile));
        const saveRun =
            this.dependencies.saveRun ?? ((run: PersistedRunDetail) => RunRepository.saveRun(run));
        ...
        for (const pstFile of readablePstFiles) {
            try {
                ...
                parserErrors.push(
                    `${pstFile.fileName}: ${error instanceof Error ? error.message : '未知 PST 解析错误'}`,
                );
            }
        }
        ...
        const items = Array.from(dedupedItems.values()).sort(
            (left, right) => (right.sentAt ?? 0) - (left.sentAt ?? 0),
        );
        ...
        const run: PersistedRunDetail = {
            ...
            items,
            message:
                parserErrors.length > 0 ? `${summaryMessage} 部分 PST 解析失败。` : summaryMessage,
