...
describe('SchemaManager', () => {
  ...
  it('migrates a 3.0 database to 4.0 after creating a backup', async () => {
    ...
    expect(copyFileSync).toHaveBeenCalledWith(DB_PATH, `${DB_PATH}.backup-3_0-to-4_0-1712304000000`);
  ...
  it('keeps the backup path when migration execution fails', async () => {
    ...
    const failure = await SchemaManager.initialize().catch((error) => error as SchemaMigrationError);
