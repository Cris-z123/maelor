...
describe('onboardingHandler', () => {
  ...
  it('logs and rethrows detect errors', async () => {
    ...
    await expect(handleDetectEmailClient({} as Electron.IpcMainInvokeEvent)).rejects.toThrow('boom');
    expect(errorSpy).toHaveBeenCalledWith('OnboardingHandler', 'Detect Outlook directory failed', {
      error: 'boom',
    });
  });
  ...
  it('validates the selected PST path and records the result', async () => {
    ...
    const recordValidation = vi.spyOn(OnboardingManager, 'recordValidation').mockReturnValue({} as never);

    await expect(handleValidateEmailPath({} as Electron.IpcMainInvokeEvent, 'C:\\Outlook')).resolves.toEqual(result);
