...
function getRegisteredHandlers(): HandlerMap {
    return new Map(
        handle.mock.calls.map(([channel, fn]) => [
            channel as string,
...
describe('registerAppIpcHandlers', () => {
    ...
    it('removes old handlers and registers override-backed handlers', async () => {
        const overrides = {
            ...
            detectOutlookDirectory: vi
                .fn()
                .mockResolvedValue({ detectedPath: 'C:\\', reason: 'found' }),
            validateOutlookDirectory: vi.fn().mockResolvedValue({
                valid: true,
                readablePstCount: 1,
            ...
            testConnection: vi
                .fn()
                .mockResolvedValue({ success: true, responseTimeMs: 1, message: 'ok' }),
        ...
        await expect(
            registered.get(IPC_CHANNELS.ONBOARDING_VALIDATE_EMAIL_PATH)?.({} as never, 'C:\\Inbox'),
        ).resolves.toEqual({
            valid: true,
            readablePstCount: 1,
            unreadablePstCount: 0,
        ...
        await expect(
            registered.get(IPC_CHANNELS.RUNS_GET_BY_ID)?.({} as never, { runId: 'run-2' }),
        ).resolves.toEqual({
            runId: 'run-2',
        });
    ...
    it('uses default dependencies for onboarding, runs, and settings actions', async () => {
        ...
        vi.spyOn(RunExecutionService.prototype, 'start').mockResolvedValue({
            success: true,
            runId: 'run-1',
            message: 'done',
        });
        ...
        await expect(
            registered.get(IPC_CHANNELS.ONBOARDING_DETECT_EMAIL_CLIENT)?.(),
        ).resolves.toEqual({
            detectedPath: 'C:\\Outlook',
            reason: 'found',
        });
        await expect(
            registered.get(IPC_CHANNELS.ONBOARDING_VALIDATE_EMAIL_PATH)?.(
                {} as never,
                'C:\\Outlook',
            ),
        ).resolves.toEqual({
        ...
        await expect(registered.get(IPC_CHANNELS.RUNS_GET_LATEST)?.()).resolves.toEqual({
            runId: 'latest',
        });
        await expect(
            registered.get(IPC_CHANNELS.RUNS_GET_BY_ID)?.({} as never, { runId: 'run-7' }),
        ).resolves.toEqual({
            runId: 'detail',
        });
        await expect(
            registered.get(IPC_CHANNELS.RUNS_LIST_RECENT)?.({} as never, undefined),
        ).resolves.toEqual([{ runId: 'recent' }]);
        ...
        expect(info).toHaveBeenCalledWith(
            'IPC',
            'Registered active onboarding, runs, and settings handlers',
        );
    });
});
