  ...
  it('classifies unreadable PST files without hiding readable ones', () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ fileName: 'readable.pst', readability: 'readable', reason: null }),
        expect.objectContaining({ fileName: 'locked.pst', readability: 'unreadable', reason: 'Access denied' }),
      ])
    );
  });
});
