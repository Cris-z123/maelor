...
function createMockFolder(messages: Array<Record<string, unknown>> = [], subfolders: Array<Record<string, unknown>> = []) {
  let cursor = 0;
...
afterEach(() => {
  ...
  (RunExecutionService as unknown as { activeRunPromise: Promise<unknown> | null }).activeRunPromise = null;
});
...
describe('RunExecutionService coverage', () => {
  ...
  it('dedupes extracted items, assigns heuristic confidence levels, and records partial parser errors', async () => {
    ...
    const service = new RunExecutionService(createDependencies({ loadPstEmails, saveRun }, pstFiles));
  ...
  it('marks the run as failed when every readable PST parse fails and releases the active lock afterward', async () => {
    const loadPstEmails = vi.fn().mockRejectedValueOnce(new Error('broken pst')).mockResolvedValueOnce([]);
    const saveRun = vi.fn<(_: PersistedRunDetail) => Promise<void>>().mockResolvedValue(undefined);
  ...
  it('walks PST folders recursively, maps supported messages, and closes the PST file', async () => {
    ...
    expect(PSTFile).toHaveBeenCalledWith('C:\\Users\\User\\Documents\\Outlook Files\\mailbox.pst');
