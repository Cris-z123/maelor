    ...
    if (normalized.includes('FROM item_evidence ie LEFT JOIN processed_emails pe')) {
      return {
        get: (itemId: string) => {
          ...
          const email = this.processedEmails.find((row) => row.email_id === evidence.email_id);
          return {
      ...
      return {
        all: (runId: string) => {
          ...
          return this.itemEvidence
            ...
            .sort((left, right) => left.item_id.localeCompare(right.item_id) || left.evidence_id.localeCompare(right.evidence_id))
            .map((row) => ({
