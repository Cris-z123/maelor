...
describe('appApi', () => {
  ...
  it('returns safe fallback values when the Electron API is unavailable', async () => {
    ...
    await expect(appApi.testConnection('https://api.openai.com/v1', 'key', 'gpt-4.1-mini')).resolves.toEqual({
      success: false,
      responseTimeMs: null,
      message: expect.stringContaining('Electron API'),
    });
  ...
  it('delegates to the Electron API with the expected request shapes', async () => {
    const electronApi = {
      onboarding: {
        getStatus: async () => ({ completed: true, currentStep: 3, readablePstCount: 1, outlookDirectory: 'C:\\Outlook' }),
        ...
        testLLMConnection: async (payload: { baseUrl: string; apiKey: string; model: string }) => ({
          success: true,
          responseTimeMs: 120,
          message: `${payload.model}:ok`,
        }),
        complete: async (payload: { directoryPath: string; baseUrl: string; apiKey: string; model: string }) => ({
    ...
    await expect(appApi.updateSettings({ aiModel: 'gpt-4.1-mini', apiKey: 'new-key' })).resolves.toBe(true);
    await expect(appApi.clearRuns()).resolves.toEqual({
      success: true,
      deletedRunCount: 3,
    });
